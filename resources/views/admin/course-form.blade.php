...
<div class="bg-white rounded-lg shadow-lg p-6">
    ...
    <form method="POST" action="{{ $mode === 'create' ? route('admin.courses.store') : route('admin.courses.update', $course->id) }}" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $course->title) }}" class="w-full px-4 py-2 border rounded" required>
            </div>
            <div>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">المحاضر</label>
                @if(auth()->user()->hasRole('teacher'))
                    <input type="hidden" name="teacher_id" value="{{ auth()->id() }}">
                    <input type="text" value="{{ auth()->user()->name }}" class="w-full px-4 py-2 border rounded bg-gray-100" readonly>
                @else
                <select name="teacher_id" class="w-full px-4 py-2 border rounded" required>
            ...
            <div>
                ...
                <select name="level" class="w-full px-4 py-2 border rounded" required>
            ...
            <div>
                ...
                <select name="language" class="w-full px-4 py-2 border rounded" required>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="price" value="{{ old('price', $course->price) }}" class="w-full px-4 py-2 border rounded" required>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">سعر مخفّض (اختياري)</label>
                <input type="number" step="0.01" name="discount_price" value="{{ old('discount_price', $course->discount_price) }}" class="w-full px-4 py-2 border rounded">
            </div>
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">وصف الدورة</label>
            <textarea name="description" rows="5" class="w-full px-4 py-2 border rounded" required>{{ old('description', $course->description) }}</textarea>
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">نبذة مختصرة</label>
            <textarea name="short_description" rows="3" class="w-full px-4 py-2 border rounded">{{ old('short_description', $course->short_description) }}</textarea>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">المتطلبات (سطر لكل متطلب)</label>
                <textarea name="requirements_text" rows="6" class="w-full px-4 py-2 border rounded" placeholder="مثال:\n- معرفة أساسية بالحاسوب\n- اتصال إنترنت جيد">{{ old('requirements_text', is_array($course->requirements) ? implode("\n", $course->requirements) : '') }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">ماذا ستتعلم؟ (سطر لكل فائدة)</label>
                <textarea name="learn_text" rows="6" class="w-full px-4 py-2 border rounded" placeholder="مثال:\n- إنشاء مشروع Laravel\n- بناء REST API">{{ old('learn_text', is_array($course->what_you_will_learn) ? implode("\n", $course->what_you_will_learn) : '') }}</textarea>
            </div>
        </div>
        ...
        <div>
            ...
            @else
            <div class="w-full px-4 py-2 border rounded bg-gray-100 font-medium">
        ...
        <div class="flex items-center space-x-3 space-x-reverse">
            <a href="{{ route('admin.courses') }}" class="px-4 py-2 rounded border">إلغاء</a>
    ...
    <div class="mt-10 p-4 bg-gray-50 rounded">
        <h3 class="font-semibold mb-4">محتوى الدورة (الأقسام والمحاضرات)</h3>
        @if($mode === 'create')
                ...
                <div class="bg-white rounded border p-4 mb-4">
                    ...
                    <div class="mt-3 bg-gray-50 rounded p-3">
                        ...
                        <form method="POST" action="{{ route('admin.sections.lessons.store', $section->id) }}" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-6 gap-3">
                            @csrf
                            <input type="text" name="title" class="px-3 py-2 border rounded md:col-span-2" placeholder="عنوان الفيديو" required>
                            <input type="text" name="duration_text" class="px-3 py-2 border rounded" placeholder="المدة (مثال: 45:00)">
                            <input type="file" name="video_file" accept="video/*" class="px-3 py-2 border rounded" placeholder="فيديو (اختياري)">
                            <input type="file" name="subtitle_file" accept=".vtt,.srt,text/vtt" class="px-3 py-2 border rounded" placeholder="ملف الترجمة (اختياري)">
                            <textarea name="description" rows="2" class="md:col-span-6 px-3 py-2 border rounded" placeholder="وصف مختصر (اختياري)"></textarea>
