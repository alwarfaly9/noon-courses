    ...
    <div class="bg-white rounded-xl shadow p-6">
        ...
        <form method="POST" action="{{ route('teacher.courses.sections.store', $course->id) }}"
            ...
            <input type="text" name="title"
                   class="flex-1 px-4 py-2 border rounded-lg focus:ring-2 focus:ring-purple-300"
                   placeholder="عنوان القسم (مثال: مقدمة، الفصل الأول...)" required>
            <button type="submit"
                    class="bg-purple-600 hover:bg-purple-700 text-white px-5 py-2 rounded-lg whitespace-nowrap">
    ...
    <div class="bg-white rounded-xl shadow p-6">
                ...
                <div class="mt-3 pt-3 border-t border-gray-200">
                    <form method="POST" action="{{ route('teacher.lessons.upload', $lesson->id) }}"
                        ...
                        <input type="file" name="file"
                               ...
                               class="md:col-span-3 border rounded px-3 py-1.5 text-sm bg-white">
                        <input type="file" name="subtitle" accept=".vtt,.srt"
                               class="md:col-span-2 border rounded px-3 py-1.5 text-sm bg-white">
                        <button type="submit"
                                class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-1.5 rounded text-sm">
                            رفع
                        </button>
                    </form>
        ...
        <div class="bg-purple-50 rounded-lg p-4">
            ...
            <form method="POST" action="{{ route('teacher.sections.lessons.store', $section->id) }}"
                ...
                @csrf
                <input type="text" name="title"
                       class="md:col-span-3 px-4 py-2 border rounded-lg focus:ring-2 focus:ring-purple-300"
                ...
                <input type="file" name="video_file" accept="video/*"
                       class="md:col-span-2 px-4 py-2 border rounded-lg bg-white">
                <input type="file" name="subtitle_file" accept=".vtt,.srt"
                       class="md:col-span-2 px-4 py-2 border rounded-lg bg-white">
                <textarea name="description" rows="2"
                          class="md:col-span-4 px-4 py-2 border rounded-lg focus:ring-2 focus:ring-purple-300"
